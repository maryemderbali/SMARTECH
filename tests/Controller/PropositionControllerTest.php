<?php

namespace App\Test\Controller;

use App\Entity\Proposition;
use Doctrine\ORM\EntityManagerInterface;
use Doctrine\ORM\EntityRepository;
use Symfony\Bundle\FrameworkBundle\KernelBrowser;
use Symfony\Bundle\FrameworkBundle\Test\WebTestCase;

class PropositionControllerTest extends WebTestCase
{
    private KernelBrowser $client;
    private EntityManagerInterface $manager;
    private EntityRepository $repository;
    private string $path = '/proposition/';

    protected function setUp(): void
    {
        $this->client = static::createClient();
        $this->manager = static::getContainer()->get('doctrine')->getManager();
        $this->repository = $this->manager->getRepository(Proposition::class);

        foreach ($this->repository->findAll() as $object) {
            $this->manager->remove($object);
        }

        $this->manager->flush();
    }

    public function testIndex(): void
    {
        $crawler = $this->client->request('GET', $this->path);

        self::assertResponseStatusCodeSame(200);
        self::assertPageTitleContains('Proposition index');

        // Use the $crawler to perform additional assertions e.g.
        // self::assertSame('Some text on the page', $crawler->filter('.p')->first());
    }

    public function testNew(): void
    {
        $this->markTestIncomplete();
        $this->client->request('GET', sprintf('%snew', $this->path));

        self::assertResponseStatusCodeSame(200);

        $this->client->submitForm('Save', [
            'proposition[idEntreprise]' => 'Testing',
            'proposition[numeroTelephone]' => 'Testing',
            'proposition[mail]' => 'Testing',
            'proposition[titre]' => 'Testing',
            'proposition[montant]' => 'Testing',
            'proposition[message]' => 'Testing',
        ]);

        self::assertResponseRedirects('/sweet/food/');

        self::assertSame(1, $this->getRepository()->count([]));
    }

    public function testShow(): void
    {
        $this->markTestIncomplete();
        $fixture = new Proposition();
        $fixture->setIdEntreprise('My Title');
        $fixture->setNumeroTelephone('My Title');
        $fixture->setMail('My Title');
        $fixture->setTitre('My Title');
        $fixture->setMontant('My Title');
        $fixture->setMessage('My Title');

        $this->manager->persist($fixture);
        $this->manager->flush();

        $this->client->request('GET', sprintf('%s%s', $this->path, $fixture->getId()));

        self::assertResponseStatusCodeSame(200);
        self::assertPageTitleContains('Proposition');

        // Use assertions to check that the properties are properly displayed.
    }

    public function testEdit(): void
    {
        $this->markTestIncomplete();
        $fixture = new Proposition();
        $fixture->setIdEntreprise('Value');
        $fixture->setNumeroTelephone('Value');
        $fixture->setMail('Value');
        $fixture->setTitre('Value');
        $fixture->setMontant('Value');
        $fixture->setMessage('Value');

        $this->manager->persist($fixture);
        $this->manager->flush();

        $this->client->request('GET', sprintf('%s%s/edit', $this->path, $fixture->getId()));

        $this->client->submitForm('Update', [
            'proposition[idEntreprise]' => 'Something New',
            'proposition[numeroTelephone]' => 'Something New',
            'proposition[mail]' => 'Something New',
            'proposition[titre]' => 'Something New',
            'proposition[montant]' => 'Something New',
            'proposition[message]' => 'Something New',
        ]);

        self::assertResponseRedirects('/proposition/');

        $fixture = $this->repository->findAll();

        self::assertSame('Something New', $fixture[0]->getIdEntreprise());
        self::assertSame('Something New', $fixture[0]->getNumeroTelephone());
        self::assertSame('Something New', $fixture[0]->getMail());
        self::assertSame('Something New', $fixture[0]->getTitre());
        self::assertSame('Something New', $fixture[0]->getMontant());
        self::assertSame('Something New', $fixture[0]->getMessage());
    }

    public function testRemove(): void
    {
        $this->markTestIncomplete();
        $fixture = new Proposition();
        $fixture->setIdEntreprise('Value');
        $fixture->setNumeroTelephone('Value');
        $fixture->setMail('Value');
        $fixture->setTitre('Value');
        $fixture->setMontant('Value');
        $fixture->setMessage('Value');

        $this->manager->remove($fixture);
        $this->manager->flush();

        $this->client->request('GET', sprintf('%s%s', $this->path, $fixture->getId()));
        $this->client->submitForm('Delete');

        self::assertResponseRedirects('/proposition/');
        self::assertSame(0, $this->repository->count([]));
    }
}
